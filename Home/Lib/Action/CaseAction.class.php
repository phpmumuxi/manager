...
class CaseAction extends CommonAction {
	...
	public function photo() {
		...
		$count = $photo->where('iscoop = 2')->count();				//总记录数
		...
		$list = $photo->where('iscoop = 2')->order('sortid desc')->limit($Page->firstRow.','.$Page->listRows)->select();
	...
	public function add_do() {
		if ($this->isPost()) {
			...
			if ($photo->create($data)) {
				...
				$this->success('添加成功',__APP__.'/Case/photo',3);
			}else {
	...
	public function edit_do() {
		...
		if ($this->isPost()) {
			...
			if ($photo->create($data)) {
				...
				$this->success('修改成功',__APP__.'/Case/photo',3);
			}else {
