...
class IndexAction extends Action {
	...
	public function main() {
		...
		if($_SESSION['ThinkUser']['ID']==39){
			$module = M('module');
			$list=$module->where('ID = 94')->select();
			$volist=$module->where('ID = 106')->select();
			// var_dump($list);exit;
	...
	public function content() {
		...
		if (!S('info')) {
			...
			S('info',$systeminfo,C('DataCache')*3600);
		}
		...
		if (!S('newslist')) {
			$news = M('news');
			$newslist = $news->order('Sortid desc')->where('Sid = 1')->limit(10)->select();
