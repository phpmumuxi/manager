...
class SystemAction extends CommonAction {
	...
	public function crm(){
		...
		//$mysql = "mysql://root:Luichi2015@@192.168.1.206:3306/crm";
		$mysql = "mysql://eachonline_crm:Eachonline_crm@2016@192.168.0.5:33066/eachonline_crm";
		$role = M("Permission","crm_",$mysql);
		$where['position_id'] = $_GET['key'];
		$count = $role->where($where)->count();			//总记录数
		...
		$p->setConfig('theme','%totalRow% %header% %nowPage%/%totalPage% 页%first% %upPage% %linkPage% %downPage% %end% %jump%');
		$show = $p->show();
		$roles = $role->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();
		$this->assign('role',$roles);
		$this->assign('page',$show);
		$this->display();	
	}

	public function crmadd(){
		$this->display();
	}

	public function crmdo(){
		$mysql = "mysql://eachonline_crm:Eachonline_crm@2016@192.168.0.5:33066/eachonline_crm";
		$role = M("Permission","crm_",$mysql);
		$data['role_id'] = 0;
		$data['url'] = $_POST['url'];
		...
		$add = $role->add($data);
		if($add){
			$this->success('新增成功',__APP__.'System/crm',2);
		}
	}
	...
	public function classmanage(){
        //请求列表
        $html = send_post('http://139.196.194.93:8080/smart/home/queyNav','');
        //print_r($html);exit;
        $list = $html['data']['navList'];
    ...
    public function managedel() {
        ...
        if ($this->isAjax()) {
            $id = I('post.id','');
            $data['sid']=$id;
            $status = send_post('http://139.196.194.93:8080/smart/home/manager/deleteCate',$data);
            if ($status) {
                parent::operating(__ACTION__,0,'删除类目成功');
                R('Public/errjson',array('ok'));
            }else {
                parent::operating(__ACTION__,1,'数据不存在');
                R('Public/errjson',array('数据不存在'));
     ...
     * 添加类目
     */
    public function manage_add_do() {
        parent::userauth(26);
        if ($this->isAjax()) {
            $data['value'] = I('post.sid','');
            $html = send_post('http://139.196.194.93:8080/smart/home/queyNav','');
            //print_r($html);exit;
            if($data['value'] ==0) {
                foreach($html['data']['navList'] as $v){
                    if($v['level']==0){
                        $data['value']=$v['value']+100;
                    }
                }
            }
            $datas['code']=$data['value'];
            $manage = send_post('http://139.196.194.93:8080/smart/home/manager/queyNavDetail', $datas);
            //print_r($manage);exit;
            $data['parentId'] = 'root';
            $data['parentCode'] = 'root';
            $data['level'] = 0;
            if(isset($manage['data']['sysCate']['parentCode'])){
                if($manage['data']['sysCate']['level']==0){
                    foreach($html['data']['navList'] as $v){
                        if($v['parentCode']==$manage['data']['sysCate']['value']){
                            $data['value']=$v['value']+1;
                        }
                    }
                    $data['parentId'] = $manage['data']['sysCate']['sid'];
                    $data['parentCode'] = $manage['data']['sysCate']['value'];
                }else{
                    foreach($html['data']['navList'] as $v){
                        if($v['parentCode']==$manage['data']['sysCate']['parentCode']){
                            $data['value']=$v['value']+1;
                        }
                    }
                    $data['parentId'] = $manage['data']['sysCate']['parentId'];
                    $data['parentCode'] = $manage['data']['sysCate']['parentCode'];
                }
                $data['level'] = 1;
            }
            $data['hasChild'] = 0;
            //$data['isUse'] = I('post.status',1);
            //$data['createTime'] = date('y-m-d h:i:s',time());
            $data['text'] = I('post.mname','','htmlspecialchars');

            $status=send_post('http://139.196.194.93:8080/smart/home/manager/sysCateInsert',$data);
            if ($status) {
                parent::operating(__ACTION__,0,'新增类目成功');
                R('Public/errjson',array('ok'));
            }else {
                parent::operating(__ACTION__,1,'新增失败');
